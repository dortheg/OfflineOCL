...
// .L peaks2D.C++ -> for Dorthea, just .L peaks2D.C (inside root)
//
// then, for SiRi:
...
// ede_bXfY; click on the respective peak to zoom in; if you zoomed
...
TH2* Peaks_EDE::NextMatrix()
{
    ...
    // build matrix name
    char tmp[128];
    snprintf(tmp, sizeof(tmp), "ede_raw_b%d_f%d", iBack, iFront);
...
void Peaks_EDE::ShowPeak(double px2d, double py2d, double pxp, double pyp)
{
    ...
    const char* peakname[nPeaks] = {"192Os_0M", "190Os_0M"};
    if( iPeak==0 )
